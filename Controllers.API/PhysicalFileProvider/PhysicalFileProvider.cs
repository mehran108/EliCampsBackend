...
namespace ELI.API.PhysicalFileProvider
{
    public class PhysicalFileProvider : PhysicalFileProviderBase
    {
        ...
        // Uploads the file(s) to the files system.
        public FileManagerResponse UploadStudentProfile(string path, IList<IFormFile> uploadFiles)
        {
            FileManagerResponse uploadResponse = new FileManagerResponse();
            try
            {
                string name = DateTime.Now.ToString("yyyyMMddHHmmss") + ContentDispositionHeaderValue.Parse(uploadFiles[0].ContentDisposition).FileName.Trim('"');
                string fullName = Path.Combine((this.contentRootPath + path), name);
                using (FileStream fs = System.IO.File.Create(fullName)) { uploadFiles[0].CopyTo(fs); fs.Flush(); }
                FileManagerDirectoryContent cwd = new FileManagerDirectoryContent();
                cwd.Name = name;
                uploadResponse.CWD = cwd;
                return uploadResponse;
            }
            catch (Exception e)
            {
